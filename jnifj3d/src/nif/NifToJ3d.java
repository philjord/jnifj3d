...
public class NifToJ3d {
	...
	private static Map<String, NifFile>			loadedFiles			= Collections
			.synchronizedMap(new WeakValueHashMap<String, NifFile>());

	//	private static Map<String, NifFile> loadedFiles = Collections.synchronizedMap(new WeakHashMap<String, NifFile>());

	private static Map<J3dNiAVObject, NifFile>	inUseJ3dRoots		= Collections
			.synchronizedMap(new WeakHashMap<J3dNiAVObject, NifFile>());

	// we can't request the same file at the same time, this tell threads to wait for each other
	private static Set<String>					loadingFiles		= Collections
	...
	private static NifJ3dHavokRoot extractHavok(NifFile nifFile) {
		if (nifFile != null) {
			...
			if (root instanceof NiNode || root instanceof BSTreeNode || root instanceof NiTriShape) {
				...
				} else if (niToJ3dData.nifVer.LOAD_VER < NifVer.VER_10_0_1_0 && root instanceof NiTriShape) {
				...
				if (niToJ3dData.nifVer.LOAD_VER < NifVer.VER_10_0_1_0) {
					...
					for (NiObject niObject : nifFile.blocks.getNiObjects()) {
						if (niObject instanceof RootCollisionNode) {
							...
							J3dRootCollisionNode jrcn = new J3dRootCollisionNode((RootCollisionNode)niObject,
									niToJ3dData);
					...
					if (hasRootCollision == false) {
						for (NiObject niObject : niToJ3dData.getNiObjects()) {
							if (niObject instanceof NiTriShape) {
								//morrowind has special case of a  NiTriShape 
								J3dNiTriShape s = new J3dNiTriShape((NiTriShape)niObject, niToJ3dData, null);
								s.getShape().setAppearance(PhysAppearance.makeAppearance());								 
								j3dNiAVObjectRoot.addChild(s);
							}
						}
					}
				} else {
