...
public class J3dNiNode extends J3dNiAVObject implements Fadable {
				...
				if (child instanceof NiNode) {
					...
					if (child.name.endsWith(" NonAccum")) {
						if (child.name.equals(niNode.name + " NonAccum")) {
							...
							System.out
									.println("accum not parent of child!! " + child.name + " " + niNode.nVer.fileName);
						}
					}
					...
					if (child instanceof NiTriBasedGeom) {
						...
						if (!(NifToJ3d.HIDE_EDITORS && isEditorMarker(niTriBasedGeom.name))) {
							...
							if (niTriBasedGeom instanceof NiTriShape) {
								...
								if (niTriBasedGeom.nVer.BS_Version == 34
									&& (niTriBasedGeom.name.toLowerCase().contains("meat") 
										&& niTriBasedGeom.name.toLowerCase().contains("cap"))
									|| (niTriBasedGeom.nVer.LOAD_VER == NifVer.VER_4_0_0_2
										&& niTriBasedGeom.name.toLowerCase().contains("tri shadow"))) {
									// too often, don't report
						...
						J3dNiParticleSystem j3dNiParticleSystem = new J3dNiParticleSystem((NiParticleSystem)child,
								niToJ3dData, textureSource);
						addChild(j3dNiParticleSystem);
					} else if (child instanceof NiAutoNormalParticles) {
						J3dNiAutoNormalParticles j3dNiAutoNormalParticles = new J3dNiAutoNormalParticles(
								(NiAutoNormalParticles)child, niToJ3dData, textureSource);
						addChild(j3dNiAutoNormalParticles);
					} else if (child instanceof NiRotatingParticles) {
						J3dNiRotatingParticles j3dNiRotatingParticles = new J3dNiRotatingParticles(
								(NiRotatingParticles)child, niToJ3dData, textureSource);
						...
						J3dNiDirectionalLight j3dNiDirectionalLight = new J3dNiDirectionalLight(
								(NiDirectionalLight)child, niToJ3dData);
	 ...
	 * @return
	 */
	public static J3dNiNode createNiNode(	NiNode niNode, NiToJ3dData niToJ3dData, TextureSource textureSource,
											boolean onlyNiNodes) {
